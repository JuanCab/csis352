...
{
public:
    bool Rsearch(const elemType& searchItem) const;
    ...
    void Rinsert(const elemType& insertItem);
...
private:
    void deleteFromTree(nodeType<elemType>* &p);
      ...
      //               from the binary search tree.
    bool recursive_search(nodeType<elemType>*, const elemType&) const;
    void recursive_insert(nodeType<elemType>* &,const elemType&);
};

template<class elemType>
void bSearchTreeType<elemType>::recursive_insert(nodeType<elemType>* &p,
                                               const elemType& insertItem)
{
   if (p==NULL)
   {
      ...
      recursive_insert(p->llink,insertItem);
   else
      recursive_insert(p->rlink,insertItem);
}

template<class elemType>
void bSearchTreeType<elemType>::Rinsert(const elemType& insertItem)
{
   recursive_insert(binaryTreeType<elemType>::root,insertItem);
}

template<class elemType>
bool bSearchTreeType<elemType>::recursive_search(nodeType<elemType>* p,
                                              const elemType& searchItem) const
{
   if (p==NULL)
      ...
      return recursive_search(p->llink,searchItem);
   else
      return recursive_search(p->rlink,searchItem);
}

template<class elemType>
bool bSearchTreeType<elemType>::Rsearch(const elemType& searchItem) const
{
   return recursive_search(binaryTreeType<elemType>::root,searchItem);
}
